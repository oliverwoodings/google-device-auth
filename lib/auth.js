...
var defaultOptions = {
  ...
  grantType: "http://oGoogleDeviceauth.net/grant_type/device/1.0",
 ...
 *
 *
 * For more indepth examples, please see the GitHub readme.md
 *
 * 
...
function GoogleDeviceAuth(options) {
  ...
  this.on(events.userCode, this._pollAuthEndpoint.bind(this));

  //Auto attempt reauth if allowed
  this.on(events.error, (function(err) {
    if (err.code == errors.refreshTokenError.code && this.config.autoAttemptReAuth) {
      this.auth();
    }
  }).bind(this));
}
...
GoogleDeviceAuth.prototype.auth = function() {
  ...
  if (this.options.clientId) {
    this._emitError(errors.missingClientId);
  }
  if (this.options.scopes.length === 0) {
    this._emitError(errors.missingScopes);
  }
  if (this.options.clientSecret) {
    this._emitError(errors.missingClientSecret);
  }
...
GoogleDeviceAuth.prototype.refresh = function() {
  ...
  if (this.options.refreshToken) {
    this._emitError(errors.missingRefreshToken);
  }
  if (this.options.clientSecret) {
    this._emitError(errors.missingClientSecret);
  }
...
/**
 * Utility - Wrapper for making post requests to Google. Handles errors and JSON parsing
